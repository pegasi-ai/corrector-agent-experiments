...
def create_langchain_agent(model_name: str):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        1. Carefully review the provided document and the reference file.
        2. Retrieve relevant factual information from the reference file.
        3. Identify any factually incorrect statements in the document based on the retrieved content.
        4. Provide accurate corrections, each with a brief reason.
...
def process_document_with_langchain(agent_executor, text: str, reference: str, model_name: str) -> Tuple[DocumentAnalysis, dict]:
    ...
    try:    
        ...
        print("Type of text:", type(text), "Length:", len(text) if isinstance(text, str) else "N/A")
        print("Type of reference:", type(reference), "Length:", len(reference) if isinstance(reference, str) else "N/A")
        print("First 500 chars of input_text:\n", input_text[:500])
