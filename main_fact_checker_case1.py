...
@tool
def retrieve_relevant_reference(query: str, reference_url: str, top_k: int = 10) -> dict:
    """
    Retrieve the most relevant sections from a website URL for the given query.
    
    Args:
        query: The text to find relevant information for
        reference_url: The website URL to extract content from
    ...
    website_content = extract_content_from_url(reference_url)
    ...
    sections = split_plaintext_into_sections(website_content)
    ...
    # Get embeddings and find most relevant sections
...
def create_langchain_agent(model_name: str):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        2. Use the 'retrieve_relevant_reference' tool to fetch the most relevant sections from the website links for the given document or section.
        3. Use only the retrieved relevant sections as your factual basis for fact-checking (do NOT use the entire content of the website links).
