...
@tool
def retrieve_relevant_reference(query: str, reference: str, top_k: int = 5) -> dict:
    """
    Retrieve the most relevant sections from the reference file for the given query.
    Returns:
        dict: {
            "relevant_chunks": [ ... ]
        }
    """
    from preprocess import split_plaintext_into_sections
    sections = split_plaintext_into_sections(reference)
    if not sections:
        return {"relevant_chunks": []}
    section_embeddings = embed_texts(sections)
    query_embedding = embed_texts([query])[0]
    similarities = np.dot(section_embeddings, query_embedding)
    top_indices = np.argsort(similarities)[-top_k:][::-1]
    relevant_chunks = [sections[i] for i in top_indices]
    return {"relevant_chunks": relevant_chunks}

@tool
def fact_check_document(document_text: str) -> dict:
    """
    ...
    return {
        "corrections": [],
        "summary": f"Fact-checking completed for document with {len(document_text)} characters"
    }
...
def create_langchain_agent(model_name: str):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        1. Carefully review the provided document.
        2. Use the 'retrieve_relevant_reference' tool to fetch the most relevant sections from the reference file for the given document or section.
        3. Use only the retrieved relevant sections as your factual basis for fact-checking (do NOT use the entire reference file).
        4. Identify any factually incorrect statements in the document based on the retrieved content.
        5. Provide accurate corrections, each with a brief reason.
        6. Focus strictly on verifiable facts — including dates, statistics, historical events, and scientific claims.
        7. Be conservative in your judgment — only flag clear, objective factual inaccuracies, not opinions or subjective language.
    ...
    # Create tools
    tools = [retrieve_relevant_reference]
...
def extract_plaintext_from_html_sections(reference: str) -> str:
    """
    Given a reference string (may contain <figure>...</figure>, <table>...</table>, and plain text),
    split into sections and join the plain text from all sections.
    """
    import re
    # Split into <figure>, <table>, and plain text sections
    ...
    for m in pattern.finditer(reference):
        ...
        if m.start() > last_end:
            before = reference[last_end:m.start()]
            before = before.strip()
            if before:
    ...
    if last_end < len(reference):
        after = reference[last_end:]
        after = after.strip()
        if after:
            sections.append(after)
    # Remove all HTML tags from each section
    cleaned = []
...
def process_document_with_langchain(agent_executor, text: str, reference: str, model_name: str) -> Tuple[DocumentAnalysis, dict]:
    """Process document using LangChain agent and return analysis with token usage"""
    try:
        # Always clean the reference string
        reference_clean = extract_plaintext_from_html_sections(reference)
...
def main():
    ...
    with col1:
        ...
        # File uploader for main document
        ...
        # Reference file uploader (docx, pdf, md)
        llm_plain_text = None  # Ensure variable is always defined
        reference_file = st.file_uploader(
            "Upload Reference File (Word, PDF, or Markdown)",
            type=['docx', 'pdf', 'md'],
            key="reference_file_uploader",
            help="Upload a .docx, .pdf, or .md file to use as reference for fact-checking."
        )
        if reference_file is not None:
            ref_filename = reference_file.name
            ref_bytes = reference_file.read()
            if ref_filename.lower().endswith('.docx'):
                import mammoth
                llm_plain_text = mammoth.extract_raw_text(io.BytesIO(ref_bytes)).value
            elif ref_filename.lower().endswith('.pdf'):
                import PyPDF2
                pdf_reader = PyPDF2.PdfReader(io.BytesIO(ref_bytes))
                llm_plain_text = ""
                for page in pdf_reader.pages:
                    llm_plain_text += page.extract_text() or ""
            elif ref_filename.lower().endswith('.md'):
                llm_plain_text = ref_bytes.decode("utf-8")
            else:
                llm_plain_text = None
            st.session_state.reference_file_content = llm_plain_text
            st.session_state.reference_file_name = ref_filename
        else:
            st.session_state.reference_file_content = None
        ...
        if uploaded_file is not None:
            ...
            if plain_text:
                ...
                with st.expander("View original text", expanded=False):
                    st.text_area("Original Content", plain_text, height=300, disabled=True, key="original_content_textarea")
                # Move Preview Reference File here
                if st.session_state.get('reference_file_content'):
                    with st.expander("Preview Reference File", expanded=False):
                        st.text_area("Reference Content", st.session_state['reference_file_content'], height=200, disabled=True, key="reference_content_textarea")

                # Get sections from LLM Output file
                sections = preprocess.split_plaintext_into_sections(plain_text) if plain_text else []
                ...
                section_options = [f"Section {i+1}: {chunk[:40]}..." for i, chunk in enumerate(sections)]
                ...
                if analyze_doc_clicked:
                    ...
                    with st.spinner("🤖 Fact-checking document... This may take a few moments."):
                        try:
                            ...
                            for chunk_text in sections:
                                # Create LangChain agent
                                agent_executor = create_langchain_agent(selected_model)
                                
                                if not chunk_text.strip():
                                    continue
                                analysis, token_usage = process_document_with_langchain(agent_executor, chunk_text, llm_plain_text, selected_model)
                ...
                if analyze_section_clicked:
                    ...
                    if not api_key_provided:
                        st.error("⚠️ Please enter the required API key in the sidebar first!")
                        return
                    chunk_text = sections[selected_section_idx]
